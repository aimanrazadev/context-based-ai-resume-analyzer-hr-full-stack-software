...
def test_delete_job_soft_deletes_and_keeps_dependents(client, db_session):
    # Create recruiter + login
    client.post(
        "/auth/signup",
        json={"email": "del_recruiter@example.com", "password": "Testpass123!", "role": "recruiter", "name": "R"},
    )
    login = client.post(
        "/auth/login", json={"email": "del_recruiter@example.com", "password": "Testpass123!", "role": "recruiter"}
    )
    ...
    client.post(
        "/auth/signup",
        json={"email": "del_candidate@example.com", "password": "Testpass123!", "role": "candidate", "name": "C"},
    )
    login_c = client.post(
        "/auth/login", json={"email": "del_candidate@example.com", "password": "Testpass123!", "role": "candidate"}
    )
    ...
    # Delete job via API (soft delete)
    d = client.delete(f"/jobs/{job_id}", headers={"Authorization": f"Bearer {token}"})
    assert d.status_code == 200

    deleted_job = db_session.query(Job).filter(Job.id == int(job_id)).first()
    assert deleted_job is not None
    assert deleted_job.status == "deleted"

    # Dependents are intentionally retained for history and recruiter visibility.
    assert db_session.query(Application).filter(Application.job_id == int(job_id)).count() == 1
    assert db_session.query(Interview).filter(Interview.application_id == app_id).count() == 1
    assert db_session.query(Resume).filter(Resume.id == resume_id).count() == 1
    assert db_session.query(Embedding).filter(Embedding.entity_type == "job", Embedding.entity_id == int(job_id)).count() >= 1
    assert db_session.query(Embedding).filter(Embedding.entity_type == "resume", Embedding.entity_id == resume_id).count() >= 1
